				...
				<tbody>
					<?php 
						foreach($items as $item) {
							$service_id = $item['id'];
							$serviceQ = $db->query("SELECT * FROM services WHERE id = '{$service_id}'");
							$service = mysqli_fetch_assoc($serviceQ);
							$sArray = explode(',',$service['sizes']);
					...
					?>
						<tr>
							...
							<td>
								<button class="btn btn-xs btn-default" onclick="update_cart('removeone','<?=$service['id'];?>','<?=$item['size'];?>');">-</button>
								<?=$item['quantity'];?>
							<?php if($item['quantity'] < $available): ?>
								<button class="btn btn-xs btn-default" onclick="update_cart('addone','<?=$service['id'];?>','<?=$item['size'];?>');">+</button>
							<?php else: ?>
								<span class="text-danger">Max</span>
							<?php endif; ?>
							</td>
							<td><?=$item['size'];?></td>
							<td><?=money($item['quantity'] * $service['price']);?></td>
						</tr>
					<?php 
						$i++;
						$item_count += $item['quantity'];
						$sub_total += ($service['price'] * $item['quantity']);
				}
					$tax = TAXRATE * $sub_total;
					$tax = number_format($tax,2);
					$grand_total = $tax + $sub_total;// for America
				 ?>
				</tbody>
			</table>
						...
						<div class="modal-body">
							<div class="row">
								<form action="thankYou.php" method="post" id="payment-form">
									...
									<input type="hidden" name="description" value="<?=$item_count.' item'.(($item_count>1)?'s':'').' from Mikes Logistics';?>">
...
<script>
	...
	function check_address(){
		...
		jQuery.ajax({
			url : '/projectpgd/admin/parsers/check_address.php',
			method : 'POST',
			data : data,
